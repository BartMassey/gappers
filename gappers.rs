...
fn simulate_hand(hand_type: HandType) -> bool {
    ...
    let mut deck: Vec<(Rank, Suit)> = ranks
        .iter()
        .flat_map(|&rank| suits.iter().map(move |&suit| (rank, suit)))
        .collect();
...
fn estimate_probabilities(n: i32) -> (f64, f64) {
    ...
    (
        gapper_flush_counts as f64 / n as f64,
        suited_flush_counts as f64 / n as f64,
    )
}
